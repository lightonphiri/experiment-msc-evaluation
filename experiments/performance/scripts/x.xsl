<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="record">
		<doc>
			...
			<field name="dc-identifier"><xsl:value-of select="header/identifier" /></field>
			<!--field column="dc-available" xpath="/record/header/datestamp" commonField="true" /-->
			<field name="dc-available"><xsl:value-of select="header/datestamp" /></field>
			<!--field column="dc-publisher" xpath="/record/header/setSpec" commonField="true" /-->
			<field name="dc-publisher"><xsl:value-of select="header/setSpec" /></field>
	...
	<xsl:template match="dc:title">
		<field name="dc-title"><xsl:value-of select="." /></field>
	</xsl:template>
	<!--field column="dc-publisher" xpath="/record/header/setSpec" commonField="true" /-->
	<xsl:template match="dc:publisher">
		<field name="dc-publisher"><xsl:value-of select="." /></field>
	</xsl:template>
	<!--field column="dc-creator" xpath="/record/metadata/dc/creator" /-->
	<xsl:template match="dc:creator">
		<field name="dc-creator"><xsl:value-of select="." /></field>
	</xsl:template>
	<!--field column="dc-subject" xpath="/record/metadata/dc/subject" /-->
	<xsl:template match="dc:subject">
		<field name="dc-subject"><xsl:value-of select="." /></field>
	</xsl:template>
	<!--field column="dc-description" xpath="/record/metadata/dc/description" /-->
	<xsl:template match="dc:description">
		<field name="dc-description"><xsl:value-of select="." /></field>
	</xsl:template>
